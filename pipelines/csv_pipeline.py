...
async def process_row(row: dict, model: str, run_id: str) -> dict:
    ...
    tax_out  = await get("taxonomy").execute(TaxonomyInput(product_name=name, model=model))
    seo_out  = await get("seo").execute(SEOInput(product_name=name, model=model))
    kpi_out  = await get("kpi").execute(KPIInput(
        original_name=name, seo_name=seo_out.seo_name,
        subcategory=tax_out.subcategory, model=model))
    desc_out = await get("description").execute(DescriptionInput(
        product_name=name, subcategory=tax_out.subcategory, sku=sku, model=model))
    ...
    return {
        **row,
        ...
        "AI_Description":     desc_out.description,
        "_tax_input":  f"Classify: {name}",        "_tax_output":  tax_out.subcategory,
        "_seo_input":  f"Rewrite for SEO: {name}", "_seo_output":  seo_out.seo_name,
        "_kpi_input":  f"KPI for: {name}",         "_kpi_output":  kpi_out.kpi_summary,
        "_desc_input": f"Describe: {name}",        "_desc_output": desc_out.description,
    }

async def stream_csv(csv_content: str, model: str = "gpt-4o-mini"):
    ...
    for coro in asyncio.as_completed(tasks):
        ...
        for agent, inp, out in [
            ...
            ("description", result.get("_desc_input",""), result.get("_desc_output","")),
        ]:
